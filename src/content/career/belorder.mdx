...
links:
  ...
  - name: Instagram
    href: https://www.instagram.com/belorder_fr/
---
...
Printing tickets has been one of the main challenges, since not all the _thermal printers_ are the same, and because we need to support `USB` and `Wifi` printing. To achieve this, we have to implement this feature natively in [](development/android)
...
With this, the web only asks for the catalogs (which uses time in the `server`), once every _X minutes_. This allows us to provide a super-fast experience: we never ask for _fresh data_ we always serve the _cached data_ and asks for `refresh` every _X minutes_.

However, this on its own wouldn't allow us to scale at the rhythm the company is growing: deploying one [](framework/nextjs) application takes some developer time to setup everything. That's why we introduced **multi-domain instances**. That is: one [](framework/nextjs) instance can take care of multiple companies. This + the use of [](database/redis) to save results of the _incremental static regeneration_ allows us to scale really fast today. We just need to tweak some values in [dashboard](#dashboard) and _voilà_: a new company has its website!

#### The Kiosk

![bordered responsive title:Belorder Kiosk used in taobento restaurant; alt:Belorder Kiosk](/career/belorder/kiosk.jpg)

The kiosk application allows customers to make orders in the restaurants, without waiting in queues. It's a [](framework/react-native) application that runs in [](development/android). The main challenge of this application was the optimization. [](development/android) is not the fastest OS, so we had to optimize the app to make sure it works as smooth as possible.
...
The mobile application is personalized for each company, and allows users to have closer relation with their favorite brands! It's yet another [](framework/react-native) app. Shares a lot of functionality with the [web](#web) (serves basically the same purpose). Using a `monorepo` for this project proved to be really helpful, since we didn't have to _re-implement_ most of the code - just work on the views.
...
For the backend, we are mainly working with [](framework/nestjs) and [](database/mongodb). We expose different `APIs` that can be consumed by the _frontends_ through clients generated using [](technology/openapi). The architecture is quite simple and easy to maintain.
