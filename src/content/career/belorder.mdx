...
summary:
  format: mdx
  raw: I'm currently embarking on an exciting journey with **Belorder**, where I'm diving deep into the world of restaurant technologies. My role at Belorder is `Full-stack engineer`.
---
...
My current role at Belorder is `Full-stack engineer`. Where _full-stack_ means really means **\*full**-stack\*. The main `full-stack engineer`, I'm not bound to a single scope: I take one problem and provide a solution for the _Frontend_, _DevOps_ and _Backend_. This way of working is great, since I'm able to have \*full-picture\* of how everything works. This also allows me to improve my skills in different areas, instead of just focusing in one.
...
For example: the `web` application and the `mobile app` application serve the same purpose (making orders), so it wouldn't make sense to _re-do_ all the business logic for each one. Both applications share a common module `core` that contains all the business logic.
...
![bordered lg expand alt:Main screen of Belorder's KDS](/career/belorder/kds.png)
...
![bordered responsive title:Belorder Kiosk used in taobento restaurant; alt:Belorder Kiosk](/career/belorder/app.png)
