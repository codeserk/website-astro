...
I started learning PHP in college, it was very easy to learn, but deep inside we knew that this language was a joke compared to other languages we were learning at that moment ([](language/java), [](language/c)/[](language/cpp)). We can't deny that we could build the easiest backend services back then, but something wasn't right.
...
- There is no easy way to create consumers for message brokers, so we had to use another language for that part, which had to make http requests to the PHP API, not great.
