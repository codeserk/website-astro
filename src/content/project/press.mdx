...
`Press` is the core library used to make this website happen. The library reads from different sources ([](technology/wordpress) or filesystem for now) and serves a [](technology/graphql) server to get all the content. It also provides some tools to transform the media (resize, optimize, etc.).
