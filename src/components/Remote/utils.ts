...
export function createComponentProxy(result: any, _components: Record<string, any> = {}) {
...
export async function markdown(
  ...
  //     renderer,
  //   })
  // //   markedPluginsSet = true
  // // }

  const marked = new Marked(markedSmartypants(), markedFootnote(), ...markedExtenstion, {
    gfm: true,
    renderer,
  })
  const content = await marked.parse(dedent(input))
  return transform(content, [swap(opts.components), sanitize(opts.sanitize)])
}

export async function html(input: string, opts: HTMLOptions = {}): Promise<string> {
  return transform(dedent(input), [sanitize(opts.sanitize), swap(opts.components)])
}
